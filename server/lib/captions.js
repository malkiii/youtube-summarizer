 ...
 */
export async function getVideoTranscript(videoId, lang = 'en') {
  try {
    ...
    let selectedCaptionTrack = captionTracks.find(track => track.languageCode === lang);
    ...
    return result.transcript.text.map(item => he.decode(item._)).join(' ');
  } catch (err) {
